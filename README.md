...
- Survey/trajectory displays: cluster survey runs and trajectories are shown in 3D and in horizontal projection, with optional uncertainty ellipses and a Rig drill-floor (`RTE`) depth reference when the selected fixed platform has a linked `PlatformRig` with `FixedPlatformProperties.DrillFloorDepth`.
